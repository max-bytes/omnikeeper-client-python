...
def get_trait_relation_list(ok_api_client: okc.OkApiClient, trait_name: str, relation_name: str, layers: [str]) -> List[Dict[str,Any]]:
    """
...
def get_trait_relation_dataframe(ok_api_client: okc.OkApiClient, trait_name: str, relation_name: str, layers: [str], keep_ciid_as_column: bool = False) -> List[Dict[str,Any]]:
    """
    Returns the specified relation for a trait
    ...
    -------
    pd.DataFrame
        result all data for a specific trait in a dataframe format
    """
...
def upsert_trait_relations_by_ciid(ok_api_client: okc.OkApiClient, trait_name: str, predicate: str, base_ciid: uuid.UUID, related_ciids_to_add: List[uuid.UUID], write_layer: str, read_layers: [str] = None) -> bool:
    """
    upserts/replaces trait relations for a specific ci, the relation should be defined when defining the trait as trait relation definition
    ...
    with client as session:
        ...
        query = gql(f"""
            mutation($readLayers: [String]!, $writeLayer: String!, $baseCIID: Guid!, $relatedCIIDsToAdd: [Guid]!) {{
            setRelationsByCIID_{prefixed_escaped_trait_name}_{predicate}(
                layers: $readLayers
                writeLayer: $writeLayer
                baseCIID: $baseCIID
                relatedCIIDsToAdd: $relatedCIIDsToAdd
        ...
        try:
            ...
            if "ciid" in result[f"setRelationsByCIID_{prefixed_escaped_trait_name}_{predicate}"]:
                return True
            else: 
                print(result)
                return False
